...
class SPEC2026(Target):
    """
    The `SPEC-CPU2026 <https://www.spec.org/cpu2026/>`_ benchmarking suite.
    ...
    - ``--benchmarks``: alias for ``--spec2026-benchmarks``
    ...
    def fetch(self, ctx: Context) -> None:
        def do_install(srcdir: str) -> None:
            ...
            ctx.log.debug("installing SPEC-CPU2026 into " + install_path)
            run(
                ctx,
        ...
        if self.source_type == "isofile":
            ...
            ctx.log.debug("mounting SPEC-CPU2026 ISO to " + mountdir)
            os.mkdir(mountdir)
            run(ctx, ["fuseiso", self.source, mountdir])
            do_install(mountdir)
            ctx.log.debug("unmounting SPEC-CPU2026 ISO")
            ...
            ctx.log.debug("extracting SPEC-CPU2026 source files")
            ...
            do_install("src")
            ctx.log.debug("removing SPEC-CPU2026 source files to save disk space")
            ...
            ctx.log.debug("cloning SPEC-CPU2026 repo")
    ...
    def _apply_patches(self, ctx: Context) -> None:
        ...
        for path in self.patches:
            ...
            if apply_patch(ctx, path, 1) and self.source_type == "installed":
                ctx.log.warning(
                    f"applied patch {path} to external SPEC-CPU2026 directory"
                )
    ...
    def run(
        ...
        if isinstance(pool, PrunPool):
            output_root = f"/local/{getpass.getuser()}/cpu2026-output-root"
        ...
        if self.force_cpu >= 0:
            if isinstance(pool, ProcessPool) and pool.parallelmax > 1:
                ctx.log.warning(
                    ...
                    "SPEC2026 because using parallel=proc with "
                    "parallelmax > 1"
                )
            else:
        ...
        if pool:
            if isinstance(pool, PrunPool):
                ...
                benchdir="benchspec/CPU2026/{{bench}}"
    ...
    def _make_spec_config(self, ctx: Context, instance: Instance) -> str:
        ...
        ctx.log.debug("writing SPEC2026 config to " + config_path)
        ...
        with open(config_path, "w") as f:
            with redirect_stdout(f):
                ...
                print(f"    COPTIMIZE          = -std=c18")
                print(f"    CXXOPTIMIZE        = -std=c++17")
                ...
                if ctx.arch not in arch_suffixes:
                    raise RuntimeError(
                        f"Architecture '{ctx.arch}' is not supported by SPEC26 target"
    ...
    def _get_benchmarks(self, ctx: Context, instance: Instance) -> Iterable[str]:
        ...
        for bset in ctx.args.benchmarks:
            for bench in self.benchmarks[bset]:
                if not hasattr(instance, "exclude_spec2026_benchmark") or not getattr(
                    instance, "exclude_spec2026_benchmark"
                )(bench):
